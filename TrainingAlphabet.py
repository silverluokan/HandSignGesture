...
# Define the paths
keras_model_path = 'alphabets_model.keras'
tflite_model_path = 'alphabets_model.tflite'
...
firebase.delete('/', 'alphabets_model')
firebase.put('/', 'alphabets_model', actions_list)
...
class SignLanguageRecognizer:
    ...
    def start_training(self):
        ...
        upload_model_to_storage(keras_model_path, 'models/alphabets_model.keras')
        upload_model_to_storage(tflite_model_path, 'models/alphabets_model.tflite')
    ...
    def train_next_word(self):
        ...
        if current_word_index < len(actions) - 1:
            ...
            print(f"Training next letter: {actions[current_word_index]}")
    ...
    def train_prev_word(self):
        ...
        if current_word_index > 0:
            current_word_index -= 1
            print(f"Training previous letter: {actions[current_word_index]}")
            self.current_sequence = 0  # Reset sequence counter
            self.current_frame_num = 0  # Reset frame counter
            self.update_status()
            # Do not start collecting data automatically
            ...
            self.status_label.configure(text="Already at the first letter.")
            self.app.update_idletasks()
